...
def test_ui_contract_chart_first_and_no_valuation():
    ...
    assert '총보수, 연' in app
    assert '기타 비용, 거래비용 및 세금 제외' in detail_markup
...
def test_normalize_total_fee():
    assert gen.normalize_fee({'data': {'total_fee': 0.0062}}) == 0.0062
    for payload in ({}, {'data': {}}, {'data': {'total_fee': -0.1}}, {'data': {'total_fee': 'nan'}}):
        with __import__('pytest').raises(ValueError):
            gen.normalize_fee(payload)
...
def test_offline_no_network_or_valuation(tmp_path, monkeypatch):
    ...
    fee_cache = json.loads(Path(gen.FEE_CACHE_FILE).read_text())
    assert set(fee_cache) == {'fees'}


def test_holdings_and_fee_fallback_and_cached_offline(tmp_path, monkeypatch):
    c = gen.EtfFactsCollector(path=tmp_path/'cache.json', fee_path=tmp_path/'fees.json')
    ...
    c.fee_cache['fees']['069500'] = {
        'total_expense_ratio_pct': 0.15, 'url': 'https://www.k-etf.com/etf/069500'}
    ...
    assert facts['total_expense_ratio_pct'] == 0.15
    assert facts['sources']['total_expense_ratio_pct']['status'] == 'cached_after_failure'
    assert_no_valuation_keys(facts)
    c.save()
    fail.reset_mock()
    c = gen.EtfFactsCollector(offline=True, path=tmp_path/'cache.json', fee_path=tmp_path/'fees.json')
    c.enrich('069500.KS', facts)
    fail.assert_not_called()
    assert facts['sources']['holdings']['status'] == 'cached'
    assert facts['sources']['total_expense_ratio_pct']['status'] == 'cached'


def test_reviewed_holdings_fallback_tolerates_unknown_weight(tmp_path, monkeypatch):
    collector = gen.EtfFactsCollector(path=tmp_path/'cache.json', fee_path=tmp_path/'fees.json')
...
def test_generated_holdings_strict_json_without_valuation():
    paths = [Path('trend_data.json'), Path(gen.CACHE_FILE), Path(gen.FEE_CACHE_FILE), *Path('detail_data').glob('*.json')]
    ...
    trend = json.loads(Path('trend_data.json').read_text())
    assert len(trend['etfs']) == 55
    assert all(row['total_expense_ratio_pct'] is not None for row in trend['etfs'])
