...
(async()=> {
  ...
  assert.ok(nodes.get('etf-head').innerHTML.includes('총보수, 연'));
  assert.ok(nodes.get('etf-body').innerHTML.includes('0.0062%'));
  ...
  click('reset');
  const feeHeader=nodes.get('etf-head').children.find(th=>th.dataset.sort==='total_expense_ratio_pct');
  click('etf-head',feeHeader);
  assert.equal(feeHeader.attrs['aria-sort'],'ascending');
  click('etf-head',feeHeader);
  assert.equal(feeHeader.attrs['aria-sort'],'descending');
  ...
  assert.equal(price.data.datasets.every(dataset=>dataset.pointRadius===0&&dataset.pointHoverRadius===0),true);
  ...
  const detailHtml=nodes.get('detail-content').innerHTML;
  assert.ok(detailHtml.indexOf('VWAP 가격 차트')<detailHtml.indexOf('<div class="facts">'),'chart is first in detail content');
  assert.equal(/\b(?:PBR|PER)\b/.test(detailHtml),false);
  assert.ok(detailHtml.includes('상위 10개 보유주식'));
