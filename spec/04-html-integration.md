...
`index.html` loads pinned Chart.js 4.4.8, `style.css` and `app.js` using deferred scripts. CSS, app and JSON fetches share the cache token `data-etf-20260912-v2`. No standalone detail page remains. Serve from any static HTTP host; ETF details are fetched on demand from `detail_data/`.
