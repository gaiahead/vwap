...
`index.html` loads pinned Chart.js 4.4.8, `style.css` and `app.js` using deferred scripts. CSS, app and JSON fetches share one cache token. No standalone detail page remains. Serve from any static HTTP host; ETF details are fetched on demand from `detail_data/`.
