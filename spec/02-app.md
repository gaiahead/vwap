...
The list ends with 20-day average volume. It shows name/ticker, category, AUM, average trading value and average volume. Issuer remains searchable and filterable but is not a list column. Average trading value and average volume are displayed as rounded integers. Detail basic facts show issuer, K-ETF's advertised annual total fee, benchmark and exposure. The fee label explicitly excludes other costs, trading costs and taxes; those values are not added or displayed. Premium/discount and holdings summaries are not basic-fact columns. The detail begins with the VWAP chart, followed by basic facts and up to ten direct stock holdings with weights. PBR and PER are not collected, generated or displayed. Missing values display a dash.

Chart range buttons are exactly `1년`, `5년`, `10년`, default `1년`, with `aria-pressed`. Bounds are measured backwards from the last available trading date by calendar years, clamping leap day to February 28 when needed. Shorter histories show all available rows. Switching ranges updates chart data, visible date bounds and row counts. VWAP calculations occur before display slicing. The x-axis always includes the latest date as its rightmost label. Selecting a date draws only a persistent black vertical line and shows the date plus integer VWAP values below the chart. Point and hover radii remain zero, so selection never draws circles. The floating tooltip is disabled. Volume Profile is not generated or displayed.
...
The VWAP price y-axis uses Chart.js `type: logarithmic`, visibly labeled 로그 스케일. Nonfinite, zero and negative VWAP values become null before reaching the chart.
